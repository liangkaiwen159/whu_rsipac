...
json_path = os.path.join(
    train_root_dir, 'instances_train.json')
imgs_path = os.path.join(train_root_dir, 'images')
whu_dataset = COCO(json_path)
whu_dataset_classes = dict([(v['id'], v['name'])
                            for k, v in whu_dataset.cats.items()])
imgs_id = whu_dataset.getImgIds()
crop_dataset_root_dir = 'E:\梁凯文\datasets\chusai_crop'
...
def crop_img(img_path, targets, size=640, gap=160):
    ...
    XY_sequence = np.concatenate(
        (X_sequence, Y_sequence, X1_sequence, Y1_sequence), -1)  # 矩阵形式的每个左上角顶点对应的左上角点和右下角点坐标 (M, N, 4)
    ...
    for target in targets:
        ...
        index_x = center_x // (size - gap) if center_x // (size -
                                                           gap) <= number_w else number_w
        index_y = center_y // (size - gap) if center_y // (size -
                                                           gap) <= number_h else number_h
        convert_targets_cls.append(cls)
        convert_targets_xy.append([max(x, XY_sequence[index_y][index_x][0]),
    ...
    for i in range(number_w + 1):
        for j in range(number_h + 1):
            ...
            cv2.rectangle(
                watch_img, XY_sequence[j][i][:2], XY_sequence[j][i][2:], (0, 255, 0), 5)
            write_img = img[XY_sequence[j][i][1]:XY_sequence[j][i]
                            [3], XY_sequence[j][i][0]:XY_sequence[j][i][2], :]
            write_path = Path((img_path.replace('release', 'crop').split(
                '.')[0]+'_'+str(j)+'_'+str(i)+'.jpg').replace('\\', "/"))
            write_mask_img = cv2.imdecode(np.fromfile(img_path.replace(
                'images', 'masks').replace('tif', 'png'), dtype=np.uint8), cv2.IMREAD_GRAYSCALE)[XY_sequence[j][i][1]:XY_sequence[j][i]
                                                                                                 [3], XY_sequence[j][i][0]:XY_sequence[j][i][2]]
            write_mask_path = Path(str(write_path).replace(
                'images', 'masks').replace('jpg', 'png'))
    ...
    for (i, j) in index_xy_list:
        cv2.rectangle(
            watch_img, XY_sequence[j][i][:2], XY_sequence[j][i][2:], (0, 0, 255), 20)

    for count, xy in enumerate(convert_targets_xy):
        cv2.rectangle(watch_img, xy[:2], xy[2:], (0, 0, 0), 20)
        i, j = index_xy_list[count]
        txt_path = (img_path.replace('release', 'crop').split(
            '.')[0]+'_'+str(j)+'_'+str(i)+'.jpg').replace('\\', "/").replace('jpg', 'txt').replace('images', 'lables')
        ...
        lables_txt.writelines(
            [str(cls), ' ', str(x), ' ', str(y), ' ', str(x1), ' ', str(y1), '\n'])
        lables_txt.close()
...
for img_id in tqdm(imgs_id):
    ...
    mask_img_path = os.path.join(train_root_dir, 'masks',
